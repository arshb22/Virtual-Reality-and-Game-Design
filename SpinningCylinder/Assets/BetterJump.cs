...
public class BetterJump : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) && transform.position.y>4.0f && transform.position.y<4.1f && (!(Input.GetKey(KeyCode.D))))
        {
    ...
    void FixedUpdate()
    {
        ...
        if (transform.position.y < 1.0f && (Input.GetKey(KeyCode.D)))
        {
            transform.position = new Vector3(transform.position.x, 1.0f, transform.position.z);
        }
        if(transform.position.y < 4.0f && !(Input.GetKey(KeyCode.D)))
        {
